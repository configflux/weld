...
class AgentWorkflowDocTest(unittest.TestCase):
    """Validate that cortex/docs/agent-workflow.md covers all retrieval surfaces."""

    def setUp(self) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        self.doc_path = repo_root / "cortex" / "docs" / "agent-workflow.md"
        self.assertTrue(
    ...
    def test_all_retrieval_commands_documented(self) -> None:
        ...
        for cmd in REQUIRED_COMMANDS:
            ...
            if f"`cortex {cmd}`" not in self.text and f"`{cmd}`" not in self.text:
                missing.append(cmd)
        self.assertEqual(
            missing,
            [],
    ...
    def test_readme_references_agent_workflow(self) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        readme = (repo_root / "cortex" / "README.md").read_text(encoding="utf-8")
        self.assertIn(
            "agent-workflow.md",
            readme,
            "cortex/README.md must reference docs/agent-workflow.md",
        )

    def test_glossary_brief_entry_updated(self) -> None:
        repo_root = Path(__file__).resolve().parents[2]
        glossary = (repo_root / "cortex" / "docs" / "glossary.md").read_text(
            encoding="utf-8"
        )
