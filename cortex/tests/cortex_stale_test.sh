...
# cortex stale test: exercises git SHA staleness tracking in discover + stale subcommand.
...
cat > "${ROOT}/.cortex/discover.yaml" <<'YAML'
sources: []
topology:
  nodes: []
  edges: []
...
cortex_in_root "${REPO_ROOT}" "${ROOT}" discover > "${ROOT}/.cortex/graph.json"

python3 -c "
import json, sys
with open('${ROOT}/.cortex/graph.json') as f:
    data = json.load(f)
...
# --- Test 2: cortex stale shows stale=false after fresh discover ---
echo "--- Test 2: cortex stale shows stale=false after fresh discover ---"
out="$(cortex_in_root "${REPO_ROOT}" "${ROOT}" stale)"
echo "${out}" | python3 -c "
import json, sys
...
print('PASS: cortex stale shows stale=false after fresh discover')
"

# --- Test 3: cortex stale shows stale=true after a new commit ---
echo "--- Test 3: cortex stale shows stale=true after a new commit ---"
cd "${ROOT}"
...
print('PASS: cortex stale shows stale=true and commits_behind=1')
"

# --- Test 4: cortex stale shows stale=false after re-discover ---
echo "--- Test 4: cortex stale shows stale=false after re-discover ---"
cd "${ROOT}"
cortex_in_root "${REPO_ROOT}" "${ROOT}" discover > "${ROOT}/.cortex/graph.json"
...
assert d['commits_behind'] == 0, f'expected commits_behind=0, got {d}'
print('PASS: cortex stale shows stale=false after re-discover')
"

# --- Test 5: cortex stale in a non-git directory ---
echo "--- Test 5: cortex stale in a non-git directory ---"
NON_GIT="${TMPDIR}/nogit"
mkdir -p "${NON_GIT}/.cortex"

# Create a graph.json without git_sha (simulates discover in non-git env)
cat > "${NON_GIT}/.cortex/graph.json" <<'JSON'
{
...
out="$(cortex_in_root "${REPO_ROOT}" "${NON_GIT}" stale)"
echo "${out}" | python3 -c "
import json, sys
d = json.load(sys.stdin)
...
print('PASS: cortex stale handles non-git directory correctly')
"

# --- Test 6: file-index.json includes meta.git_sha ---
echo "--- Test 6: file-index.json includes meta.git_sha ---"
cd "${ROOT}"
cortex_in_root "${REPO_ROOT}" "${ROOT}" build-index 2>/dev/null

python3 -c "
import json
with open('${ROOT}/.cortex/file-index.json') as f:
    data = json.load(f)
...
out="$(cortex_in_root "${REPO_ROOT}" "${ROOT}" find README)"
echo "${out}" | python3 -c "
import json, sys
d = json.load(sys.stdin)
...
echo ""
echo "PASS: all cortex_stale tests passed"
