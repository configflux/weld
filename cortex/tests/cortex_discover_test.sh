...
# cortex_discover test: runs AST discovery against the real codebase and validates counts.
#
# This test adapts to either the monorepo or a standalone cortex repo.
...
# If the repo has no .cortex/discover.yaml yet (e.g. fresh standalone clone),
# bootstrap one via `cortex init` so that `discover` has sources to scan.
...
if [[ ! -f "${ROOT}/.cortex/discover.yaml" ]]; then
  mkdir -p "${ROOT}/.cortex"
  cortex_in_root "${ROOT}" "${ROOT}" init --output "${ROOT}/.cortex/discover.yaml" \
    > /dev/null 2>&1
  CREATED_DISCOVER="true"
fi

cortex_in_root "${ROOT}" "${ROOT}" discover > "${TMPFILE}"

# Clean up the bootstrapped discover.yaml if we created it
if [[ "${CREATED_DISCOVER}" == "true" ]]; then
  rm -f "${ROOT}/.cortex/discover.yaml"
  rmdir "${ROOT}/.cortex" 2>/dev/null || true
fi
...
if is_monorepo:
    ...
    if 'package:cortex' not in nodes:
        errors.append('package:cortex not found')
    if 'file:cortex/discover' not in nodes:
        errors.append('file:cortex/discover not found')
    if 'file:cortex/repo_boundary' not in nodes:
        errors.append('file:cortex/repo_boundary not found')
    if 'doc:cortex-guide/onboarding' not in nodes:
        errors.append('doc:cortex-guide/onboarding not found')

    for legacy in ['tool:cortex_discover', 'tool:cortex_file_index', 'tool:cortex_graph', 'tool:cortex_init']:
        if legacy in nodes:
    ...
    # --- Standalone cortex repo ---
    # The standalone repo contains cortex/ source code. Discovery finds nodes
    # from whatever strategies cortex init auto-detected (which may include
