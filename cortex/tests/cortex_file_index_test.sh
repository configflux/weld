...
# cortex file index tests: exercises index building and find subcommand.
...
mkdir -p "${ROOT}/.cortex/internal"
echo "should be excluded" > "${ROOT}/.cortex/internal/meta.py"
...
kg_cmd build-index 2>/dev/null

# Verify file-index.json was created
if [[ ! -f "${ROOT}/.cortex/file-index.json" ]]; then
...
python3 -c "
import json, sys
_raw = json.load(open('${ROOT}/.cortex/file-index.json'))
...
for path in idx:
    ...
    assert '.cortex' not in parts, f'.cortex should be excluded: {path}'
    for p in parts:
...
out="$(kg_cmd find zzzznonexistent42)"
echo "${out}" | python3 -c "
import json, sys
d = json.load(sys.stdin)
...
echo ""
echo "PASS: all cortex_file_index tests passed"
