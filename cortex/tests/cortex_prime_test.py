"""Tests for the cortex prime command."""
...
from cortex.prime import prime
...
class PrimeNoCortexDirTest(unittest.TestCase):
    """When .cortex/ does not exist, suggest cortex init."""

    def test_suggests_init(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            output = prime(Path(td))
            self.assertIn("cortex init", output)
...
class PrimeMissingGraphTest(unittest.TestCase):
    ...
    def test_suggests_discover(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            cortex_dir.mkdir()
            (cortex_dir / "discover.yaml").write_text(_minimal_discover_yaml())
            output = prime(root)
            self.assertIn("cortex discover", output)
...
class PrimeMissingFileIndexTest(unittest.TestCase):
    ...
    def test_suggests_build_index(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            cortex_dir.mkdir()
            (cortex_dir / "discover.yaml").write_text(_minimal_discover_yaml())
            (cortex_dir / "graph.json").write_text(_minimal_graph(
...
class PrimeAllPresentTest(unittest.TestCase):
    ...
    def test_reports_ok(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            (cortex_dir / "graph.json").write_text(_minimal_graph(nodes=nodes))
            (cortex_dir / "file-index.json").write_text("{}")
            # Also create agent integration to suppress that hint
            claude_dir = root / ".claude" / "commands"
            claude_dir.mkdir(parents=True)
            (claude_dir / "cortex.md").write_text("test")
            output = prime(root)
...
class PrimeAgentIntegrationHintTest(unittest.TestCase):
    ...
    def test_suggests_bootstrap(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            (cortex_dir / "file-index.json").write_text("{}")
            output = prime(root)
            self.assertIn("cortex bootstrap", output)
