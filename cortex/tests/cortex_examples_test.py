...
Validates that:
  ...
  - cortex discover runs successfully against each example
...
from cortex.strategies._helpers import StrategyResult  # noqa: E402
...
class ExamplesDirectoryStructureTest(unittest.TestCase):
    ...
    def test_fastapi_discover_yaml_exists(self) -> None:
        self.assertTrue((_FASTAPI_DIR / ".cortex" / "discover.yaml").is_file())
    ...
    def test_custom_strategy_discover_yaml_exists(self) -> None:
        self.assertTrue((_CUSTOM_DIR / ".cortex" / "discover.yaml").is_file())

    def test_custom_strategy_plugin_exists(self) -> None:
        self.assertTrue(
            (_CUSTOM_DIR / ".cortex" / "strategies" / "todo_comment.py").is_file()
        )
...
class CustomStrategyLoadTest(unittest.TestCase):
    ...
    def _load_strategy(self):
        strategy_path = (
            _CUSTOM_DIR / ".cortex" / "strategies" / "todo_comment.py"
        )
...
class DiscoverIntegrationTest(unittest.TestCase):
    """cortex discover must run successfully against each example."""

    def _run_discover(self, example_dir: Path) -> dict:
        env = os.environ.copy()
        env["PYTHONPATH"] = str(_repo_root)
        proc = subprocess.run(
            [sys.executable, "-m", "cortex", "discover"],
            cwd=str(example_dir),
            capture_output=True,
            text=True,
            timeout=30,
        ...
        self.assertEqual(
            proc.returncode, 0,
            f"cortex discover failed in {example_dir.name}: {proc.stderr}",
        )
