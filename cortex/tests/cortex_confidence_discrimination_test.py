"""Tests for confidence discrimination across cortex extraction strategies.
...
from cortex.ranking import CONFIDENCE_RANK  # noqa: E402
...
class ConfidenceContractConsistencyTest(unittest.TestCase):
    ...
    def test_at_least_three_strategies_emit_non_definite(self) -> None:
        ...
        from cortex.strategies.boundary_entrypoint import extract as be_extract
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            svc = root / "svc"
            svc.mkdir()
        ...
        from cortex.strategies.typescript_exports import extract as ts_extract
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            src = root / "src"
            src.mkdir()
        ...
        from cortex.strategies.markdown import extract as md_extract
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            docs = root / "docs"
            docs.mkdir()
