"""Tests for the cortex doctor diagnostic command."""
...
from cortex.doctor import doctor
...
def _setup_dir(root, n_sources=2, nodes=None, meta=None, mcp=False, yaml_strategies=None):
    (root / ".cortex").mkdir(exist_ok=True)
    (root / ".cortex" / "discover.yaml").write_text(
...
class DoctorGraphTest(unittest.TestCase):
    def test_ok_with_counts(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            (root / ".cortex" / "graph.json").write_text(_minimal_graph(nodes=nodes, edges=edges, meta={"schema_version": 4}))
            results = doctor(root)
    ...
    def test_fail_missing(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            (root / ".cortex").mkdir()
            (root / ".cortex" / "discover.yaml").write_text(_minimal_discover_yaml(2))
            results = doctor(root)
...
class DoctorStalenessTest(unittest.TestCase):
    def test_warn_stale(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch("cortex.doctor.is_git_repo", return_value=True), \
                 patch("cortex.doctor.get_git_sha", return_value="bbb"), \
                 patch("cortex.doctor.commits_behind", return_value=3):
                results = doctor(root)
    ...
    def test_ok_fresh(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            _setup_dir(root, meta={"schema_version": 4, "git_sha": "abc123"},
                       nodes={"n0": {"id": "n0", "type": "file", "label": "n0", "props": {}}})
            with patch("cortex.doctor.is_git_repo", return_value=True), \
                 patch("cortex.doctor.get_git_sha", return_value="abc123"), \
                 patch("cortex.doctor.commits_behind", return_value=0):
                results = doctor(root)
...
class DoctorTreeSitterTest(unittest.TestCase):
    def test_ok_available(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch("cortex.doctor._check_tree_sitter_language", return_value=True):
                results = doctor(root)
    ...
    def test_warn_missing(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _setup_dir(root, n_sources=1, yaml_strategies=["tree_sitter"])
            with patch("cortex.doctor._check_tree_sitter_language", return_value=False):
                results = doctor(root)
...
class DoctorPythonVersionTest(unittest.TestCase):
    ...
    def test_warn_old(self):
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            _setup_dir(root, n_sources=1)
            with patch("cortex.doctor.sys") as mock_sys:
...
class DoctorExitCodeTest(unittest.TestCase):
    def test_exit_zero_no_fail(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with patch("cortex.doctor.is_git_repo", return_value=True), \
                 patch("cortex.doctor.get_git_sha", return_value="abc"), \
                 patch("cortex.doctor.commits_behind", return_value=0):
                results = doctor(root)
...
class DoctorMainExitCodeTest(unittest.TestCase):
    def test_main_exit_fail(self):
        from cortex.doctor import main as doctor_main
        with tempfile.TemporaryDirectory() as td:
            output = io.StringIO()
            with patch("sys.stdout", output):
