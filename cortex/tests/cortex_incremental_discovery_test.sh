...
# cortex_incremental_discovery_test: validates incremental discovery with
...
setup_project() {
  ...
  # Write discover.yaml
  cat > "${project_dir}/.cortex/discover.yaml" <<'YAML'
sources:
  - glob: "src/*.py"
    type: file
...
# ---------------------------------------------------------------------------
# Helper: run cortex discover with given flags
# ---------------------------------------------------------------------------

run_discover() {
  local project_dir="$1"
  shift
  cortex_in_root "${ROOT}" "${project_dir}" discover "$@" "${project_dir}"
}
...
STATE_FILE="${PROJECT}/.cortex/discovery-state.json"
...
cp "${GRAPH_FULL}" "${PROJECT}/.cortex/graph.json"
...
# Save graph for incremental to read
cp "${GRAPH_BEFORE}" "${PROJECT}/.cortex/graph.json"
...
echo "NOT VALID JSON {{{" > "${PROJECT}/.cortex/discovery-state.json"
...
python3 -c "
import json, sys
with open('${PROJECT}/.cortex/discovery-state.json') as f:
    state = json.load(f)
if state.get('version') != 1:
...
# Save graph
run_discover "${PROJECT}" --full > "${PROJECT}/.cortex/graph.json"
...
mkdir -p "${PROJECT}/.cortex"
cat > "${PROJECT}/.cortex/discovery-state.json" <<'JSON'
{
  "version": 999,
