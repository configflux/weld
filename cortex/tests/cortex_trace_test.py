"""Tests for ``cortex trace`` -- protocol-aware capability path surface.

``cortex trace`` returns a coherent cross-boundary slice for a task area or
...
context. Per project-xoq.2.2 the surface must:
  ...
  - model output around service / contract / interface / boundary /
    verification links
  - reuse existing graph semantics (classification from ``cortex.brief``,
...
from cortex.trace import TRACE_VERSION, main as trace_main, trace  # noqa: E402
...
class TraceReuseSemanticsTest(unittest.TestCase):
    """Trace reuses ``cortex.brief`` classification rather than inventing
    ...
    def test_route_with_protocol_promoted_to_interface(self) -> None:
        ...
        # interface in ``cortex.brief`` and must do the same here.
        nodes = {
            "service:web": {
