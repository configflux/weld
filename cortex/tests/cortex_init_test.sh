...
# cortex_init test: validates the cortex init bootstrap command.
# Tests:
...
#   3. Generated YAML is parseable by cortex/_yaml.py
...
# --- Detect repo context ---
# Monorepo has services/ and apps/ directories; standalone has only cortex/.
IS_MONOREPO="false"
if [[ -d "${ROOT}/services" && -d "${ROOT}/apps" ]]; then
  IS_MONOREPO="true"
fi

# --- Test 1: cortex init on the real repo produces valid discover.yaml ---
echo "Test 1: cortex init on real repo..."
REAL_OUT="${TMPDIR}/real_repo_out"
mkdir -p "${REAL_OUT}/.cortex"

cortex_in_root "${ROOT}" "${ROOT}" init --output "${REAL_OUT}/.cortex/discover.yaml" \
  > "${TMPDIR}/real_stdout.txt" 2>&1

if [[ ! -f "${REAL_OUT}/.cortex/discover.yaml" ]]; then
  echo "FAIL: discover.yaml not created for real repo"
  errors=$((errors + 1))
else
  # Validate it is parseable by cortex/_yaml.py
  IS_MONOREPO_PY="${IS_MONOREPO}" python3 -c "
import sys, os; sys.path.insert(0, '${ROOT}')
from cortex._yaml import parse_yaml
from pathlib import Path

text = Path('${REAL_OUT}/.cortex/discover.yaml').read_text()
...
if is_monorepo:
    ...
    # Standalone cortex repo: test fixtures contain framework
...
# --- Test 2: cortex init on bare Python directory ---
echo "Test 2: cortex init on bare Python directory..."
...
cortex_in_root "${ROOT}" "${BARE}" init --output "${BARE}/.cortex/discover.yaml" \
  > "${TMPDIR}/bare_stdout.txt" 2>&1

if [[ ! -f "${BARE}/.cortex/discover.yaml" ]]; then
...
text = Path('${BARE}/.cortex/discover.yaml').read_text()
data = parse_yaml(text)
...
mkdir -p "${GUARD}/.cortex"
echo "# existing config" > "${GUARD}/.cortex/discover.yaml"
cat > "${GUARD}/app.py" << 'PYEOF'
print("hello")
PYEOF

if cortex_in_root "${ROOT}" "${GUARD}" init --output "${GUARD}/.cortex/discover.yaml" \
  > "${TMPDIR}/guard_stdout.txt" 2>&1; then
  # Should have exited non-zero or the file should still be the original
  content=$(cat "${GUARD}/.cortex/discover.yaml")
...
cortex_in_root "${ROOT}" "${GUARD}" init --output "${GUARD}/.cortex/discover.yaml" --force \
...
mkdir -p "${GITVISIBLE}/.cortex" "${GITVISIBLE}/toolkit" "${GITVISIBLE}/tracked_ignored" "${GITVISIBLE}/ignored_only"
...
cortex_in_root "${ROOT}" "${GITVISIBLE}" init --output "${GITVISIBLE}/.cortex/discover.yaml" \
...
text = Path('${GITVISIBLE}/.cortex/discover.yaml').read_text()
data = parse_yaml(text)
...
print('PASS: cortex init follows Git-visible files and skips ignored untracked trees')
...
echo "PASS: all cortex_init tests passed"
