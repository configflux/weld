"""Tests for the cortex scaffold command and helper logic."""
...
from cortex.scaffold import scaffold_template
...
class ScaffoldTemplateTest(unittest.TestCase):
    ...
    def test_local_strategy_default_output_path(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(output_path, root / ".cortex" / "strategies" / "smoke_test.py")
    ...
    def test_external_adapter_default_output_path(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertEqual(output_path, root / ".cortex" / "adapters" / "demo_adapter.py")
            self.assertTrue(output_path.is_file())
    ...
    def test_existing_file_requires_force(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            root = Path(td)
            output_path = root / ".cortex" / "strategies" / "smoke_test.py"
...
class ScaffoldCliDispatchTest(unittest.TestCase):
    ...
    def test_cli_dispatches_scaffold(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            written = root / ".cortex" / "strategies" / "demo.py"
            self.assertTrue(written.is_file())
            self.assertIn("Wrote .cortex/strategies/demo.py", output.getvalue())
