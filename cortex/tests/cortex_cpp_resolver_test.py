"""Tests for C++ cross-file include resolver (layer 2 of cortex-cpp-ros2).
...
def _fake_parse(file_path, language, queries):  # noqa: ARG001
    """Stand-in for tree_sitter._parse_file_symbols.

    Returns hand-rolled symbol/import lists keyed by filename so we can
    drive the resolver deterministically without a real grammar.
    """
...
def _fake_call_edges(file_path, rel_path, language, queries):  # noqa: ARG001
    """Stand-in for tree_sitter._extract_call_edges.

    Mimics the layer-1 output: emits a ``<file>`` caller node and an
    edge to a ``symbol:unresolved:<callee>`` sentinel for each call site
    in the source file. Module path matches the production helper.
    """
    from cortex.strategies.tree_sitter import _ts_module_from_path
    ...
    if name == "main.cpp":
        callees = [
            ...
            "add",
            "items",
            # Synthetic callee that no included header defines so the
            # resolver leaves it as an unresolved sentinel.
            "external_unresolvable_call",
        ]
    elif name == "foo.cpp":
        # Out-of-class definitions: emit definition nodes too.
...
class CppIncludeResolverFixtureTest(unittest.TestCase):
    ...
    def _run(self, glob: str = "**/*.cpp"):
        ...
        with mock.patch.object(tree_sitter, "TREE_SITTER_AVAILABLE", True), \
             mock.patch.object(
                 tree_sitter, "_parse_file_symbols", side_effect=_fake_parse
             ), \
             mock.patch.object(
                 tree_sitter, "_extract_call_edges", side_effect=_fake_call_edges
             ):
...
class CppIncludeResolverHeaderResolutionTest(unittest.TestCase):
    ...
    def test_resolve_relative_to_file_dir(self) -> None:
        from cortex.strategies.tree_sitter import _resolve_cpp_include
