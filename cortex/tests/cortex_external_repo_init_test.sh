...
# Validates cortex init against realistic external-repo fixtures:
...
# ---------------------------------------------------------------------------
# Helper: run cortex init on a fixture and return the discover.yaml path
# ---------------------------------------------------------------------------
init_fixture() {
  local name="$1"
  local fixture_dir="${FIXTURES}/${name}"
  local out_dir="${TMPDIR}/${name}"
  mkdir -p "${out_dir}/.cortex"

  cortex_in_root "${ROOT}" "${fixture_dir}" init \
    --output "${out_dir}/.cortex/discover.yaml" --force \
    > "${TMPDIR}/${name}_stdout.txt" 2>&1

  echo "${out_dir}/.cortex/discover.yaml"
}
...
profiles = {}
for name, path in [
    ('fastapi_project', '${TMPDIR}/fastapi_project/.cortex/discover.yaml'),
    ('django_project', '${TMPDIR}/django_project/.cortex/discover.yaml'),
    ('go_project', '${TMPDIR}/go_project/.cortex/discover.yaml'),
    ('rust_project', '${TMPDIR}/rust_project/.cortex/discover.yaml'),
]:
...
text = Path('${TMPDIR}/fastapi_project/.cortex/discover.yaml').read_text()
data = parse_yaml(text)
...
if [[ ! -f "${RS_YAML}" ]]; then
  ...
  python3 -c "
import sys; sys.path.insert(0, '${ROOT}')
from cortex._yaml import parse_yaml; from pathlib import Path
...
echo "PASS: all kg_external_repo_init tests passed"
