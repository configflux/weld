...
def _cortex_root() -> Path:
    return _repo_root() / "cortex"

def _real_cli_commands() -> set[str]:
    """Parse the canonical CLI help string for command names."""
    from cortex.cli import _HELP  # noqa: WPS433 — intentional runtime import
...
def _real_strategy_files() -> set[str]:
    ...
    strat_dir = _cortex_root() / "strategies"
    return {
        p.stem
...
def _extract_cli_invocations(text: str) -> list[str]:
    """Extract ``cortex <cmd>`` and ``python -m cortex <cmd>`` references."""
    commands: list[str] = []
    code_blocks = re.findall(r"```(?:bash|sh)?\n(.*?)```", text, re.DOTALL)
    for block in code_blocks:
        commands.extend(
            re.findall(
                r"(?m)^\s*(?:python\s+-m\s+)?cortex\s+([a-z][\w-]*)\b",
                block,
            ),
        )
    return commands
...
def _extract_backtick_commands(text: str) -> list[str]:
    """Extract cortex command references in backtick spans."""
    return re.findall(r"`(?:python\s+-m\s+)?cortex\s+([a-z][\w-]*)[^`]*`", text)
...
class OnboardingDocValidationTest(unittest.TestCase):
    """Ensure cortex/docs/onboarding.md references real commands and concepts."""

    def setUp(self) -> None:
        self.path = _cortex_root() / "docs" / "onboarding.md"
    ...
    def test_template_paths_exist(self) -> None:
        self.assertIn("cortex scaffold", self.text)

class CookbookDocValidationTest(unittest.TestCase):
    """Ensure cortex/docs/strategy-cookbook.md references real commands/templates."""

    def setUp(self) -> None:
        self.path = _cortex_root() / "docs" / "strategy-cookbook.md"
        self.assertTrue(self.path.exists(), f"Missing doc: {self.path}")
        self.text = self.path.read_text(encoding="utf-8")
        self.real_commands = _real_cli_commands()
    ...
    def test_template_copy_paths_exist(self) -> None:
        self.assertIn("cortex scaffold local-strategy", self.text)
        self.assertIn("cortex scaffold external-adapter", self.text)
    ...
    def test_normalized_metadata_fields_match_contract(self) -> None:
        ...
        from cortex.contract import AUTHORITY_VALUES, CONFIDENCE_VALUES, ROLE_VALUES
...
class ReadmeDocValidationTest(unittest.TestCase):
    """Ensure cortex/README.md references real commands and docs."""

    def setUp(self) -> None:
        self.path = _cortex_root() / "README.md"
    ...
    def test_adr_links_exist(self) -> None:
        ...
        for link in links:
            self.assertTrue(
                (_cortex_root() / "docs" / "adr" / link).exists(),
                f"README.md links to missing ADR: docs/adr/{link}",
            )

class GlossaryDocValidationTest(unittest.TestCase):
    """Ensure cortex/docs/glossary.md is consistent with the contract."""

    def setUp(self) -> None:
        self.path = _cortex_root() / "docs" / "glossary.md"
        self.assertTrue(self.path.exists(), f"Missing: {self.path}")
        self.text = self.path.read_text(encoding="utf-8")
    ...
    def test_brief_entry_is_current(self) -> None:
        ...
        self.assertNotIn(
            ...
            self.text,
            "Glossary Brief entry should reflect that cortex brief is implemented",
        )

    def test_authority_values_match_contract(self) -> None:
        """Authority values listed in glossary must match contract."""
        from cortex.contract import AUTHORITY_VALUES
    ...
    def test_confidence_values_match_contract(self) -> None:
        ...
        from cortex.contract import CONFIDENCE_VALUES
    ...
    def test_role_values_match_contract(self) -> None:
        ...
        from cortex.contract import ROLE_VALUES
...
class CrossDocConsistencyTest(unittest.TestCase):
    ...
    def setUp(self) -> None:
        cortex_dir = _cortex_root()
        self.onboarding = (cortex_dir / "docs" / "onboarding.md").read_text("utf-8")
        self.cookbook = (cortex_dir / "docs" / "strategy-cookbook.md").read_text("utf-8")
        self.readme = (cortex_dir / "README.md").read_text("utf-8")
