"""Tests for the copyable template files shipped with cortex.
...
class ExternalAdapterIntegrationTest(unittest.TestCase):
    ...
    def test_runs_via_external_json_runner(self) -> None:
        from cortex.discover import _run_external_json
...
class MarkdownTemplateExistsTest(unittest.TestCase):
    ...
    def test_cortex_readme_not_empty(self) -> None:
        content = (_TEMPLATES_DIR / "cortex_readme.md").read_text(encoding="utf-8")
    ...
    def test_cortex_cmd_claude_not_empty(self) -> None:
        content = (_TEMPLATES_DIR / "cortex_cmd_claude.md").read_text(encoding="utf-8")
        self.assertGreater(len(content.strip()), 100)

    def test_cortex_skill_codex_template_exists(self) -> None:
        self.assertTrue((_TEMPLATES_DIR / "cortex_skill_codex.md").is_file())

    def test_cortex_skill_codex_not_empty(self) -> None:
        content = (_TEMPLATES_DIR / "cortex_skill_codex.md").read_text(encoding="utf-8")
        self.assertGreater(len(content.strip()), 100)
...
class TemplateDocReferencesTest(unittest.TestCase):
    ...
    def test_cookbook_references_templates(self) -> None:
        ...
        self.assertIn(
            "cortex scaffold local-strategy", text,
            "strategy-cookbook.md must reference local strategy scaffolding",
        )
        self.assertIn(
            "cortex scaffold external-adapter", text,
    ...
    def test_onboarding_references_templates(self) -> None:
        ...
        self.assertIn(
            "cortex scaffold", text,
            "onboarding.md must reference cortex scaffolding",
        )
