"""``cortex trace`` -- protocol-aware capability path surface.

``cortex trace`` returns the shortest cross-boundary slice for a capability
or known node. It is the protocol-aware companion to ``cortex brief`` v2:
...
Per ADR 0018 and project-xoq.2.2 this surface MUST:
  ...
  - reuse the existing graph semantics -- node classification is
    delegated to ``cortex.brief._classify_node`` so we never invent a second
    interaction model;
...
from cortex.brief import _classify_node
from cortex.ranking import rank_key as _rank_key
from cortex.warnings import check_confidence_gaps, check_freshness, check_partial_coverage
...
# ``cortex.brief._classify_node`` so the route-with-protocol promotion rule
# applies here too.
...
def _bucket_for(node: dict) -> str | None:
    ...
    :func:`cortex.brief._classify_node` so a ``route`` carrying ``protocol``
    ...
    # from the trace slice -- ``cortex brief`` is the right surface for
...
def main(argv: list[str] | None = None) -> None:
    """CLI entry point for ``cortex trace``."""
    ...
    parser.add_argument(
        ...
        help="Search term (tokenized like `cortex query`); omit when --node is set",
    )
    parser.add_argument(
