"""Template: project-local cortex strategy.

Copy this file to ``.cortex/strategies/<name>.py`` and customize it for your
repository.  The ``extract()`` function is the only required entry point.

How it works:
  - ``cortex discover`` resolves strategies by name from ``discover.yaml``.
  - Project-local strategies in ``.cortex/strategies/`` take priority over
    bundled ones in ``cortex/strategies/``.
...
Example ``discover.yaml`` entry for this template::
    ...
    sources:
      - strategy: my_strategy   # matches .cortex/strategies/my_strategy.py
