# Cortex Onboarding Guide

This guide describes how to adopt `cortex` in a new repository when the goal is
to help a human or LLM agent work with the whole codebase, not just parse
source files.

## Onboarding goal

The outcome of onboarding is a repository where `cortex` can answer:
...
`cortex` should model all of them when they exist. A good `discover.yaml` is
organized around the repository's real surfaces, not just around file
extensions.

## 2. Bootstrap the starter config

If `cortex` is installed in your environment, use `cortex ...` directly. If
you are working from a raw source checkout without installing it first, use
`python -m cortex ...` as the compatibility path.
...
Then treat the generated `.cortex/discover.yaml` as a draft, not a finished
configuration.
...
3. project-local strategy under `.cortex/strategies/`
4. external adapter command that emits normalized cortex JSON
...
Use `cortex scaffold local-strategy <name>` and
`cortex scaffold external-adapter <name>` to bootstrap bundled templates for
...
The direction for `cortex` is not only broader extraction. Over time, onboarded
...
If the best source of truth is already outside `cortex`, do not fight it.
...
The direction for `cortex` is to support these through a normalized external JSON
