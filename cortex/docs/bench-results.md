...
Token cost comparison across retrieval modes for the prompt fixture in `cortex/tests/bench/prompts.yaml`. See `cortex/bench/runner.py` for the harness.

Tokenizer: `bytes/4 fallback`

| id  | category   | grep tokens | cortex CLI tokens | cortex MCP tokens | CLI reduction | MCP reduction |
|-----|------------|-------------|---------------|---------------|---------------|---------------|
...
- cortex CLI vs grep: median 86%, P90 98%
- cortex MCP vs grep: median 86%, P90 98%
