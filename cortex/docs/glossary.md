...
Any repository object that `cortex` models directly, such as a source file, route,
...
The `type` field on a cortex node. Defined in `VALID_NODE_TYPES`.
...
The `type` field on a cortex edge. Defined in `VALID_EDGE_TYPES`.
...
A repo-local bridge that turns external analysis output into normalized cortex
nodes and edges without requiring that logic to live in a bundled strategy.

## Brief

The high-level agent-facing retrieval surface. `cortex brief` returns a compact,
