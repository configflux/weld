# Cortex Strategy Cookbook

This cookbook explains how to choose and extend `cortex` extraction paths without
...
3. project-local strategy in `.cortex/strategies/`
...
Do **not** require `cortex` to understand every C++ repository natively.

Preferred path:

1. reuse the repo's existing compile database or clang-based analysis
2. emit normalized cortex JSON from a repo-local command
...
Use `.cortex/strategies/<name>.py` when:
...
Use an external adapter when the best analyzer already exists outside `cortex` --
...
print valid cortex JSON to stdout.  Invalid output is rejected gracefully.
...
When extending `cortex`, optimize for questions an agent asks during real work:
