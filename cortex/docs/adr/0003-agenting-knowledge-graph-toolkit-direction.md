# 0003. Agenting Knowledge Graph Toolkit Direction
...
The current `cortex` tool already does more than source-code AST extraction.
...
The intended direction for `cortex` is not "more AST support" in isolation. The
tool should become a portable, whole-codebase, agent-facing knowledge graph
...
1. **Agent usefulness over parser novelty** -- the primary question is not
   whether `cortex` can parse one more language, but whether it can guide an agent
   through a real repository safely and efficiently.
2. **Whole-codebase coverage** -- `cortex` must continue to model code, docs,
   infrastructure, build surfaces, policy, tests, and operations rather than
   collapsing into a code-only AST extractor.
3. **Portable toolkit posture** -- projects must be able to adopt `cortex` from a
...
5. **Extensibility for legacy and polyglot repos** -- maintainers should be
   able to onboard unusual repositories without forking `cortex`.
...
**C. Treat `cortex` as a portable, whole-codebase, agenting toolkit with
...
`cortex` will evolve as a **portable agenting knowledge graph toolkit** with the
following defaults:

- `cortex` remains a portable toolkit first, not a packaging-heavy standalone
  product.
- `discover.yaml` remains the control plane for project onboarding.
- Whole-codebase coverage is a core requirement. `cortex` must continue to model
...
`cortex` will expand beyond the current artifact vocabulary to support these
...
`cortex` will add a high-level agent retrieval surface:

`cortex brief <term|node|path>`

`cortex brief` should return a compact context packet for an agent that includes:
...
`cortex` will add a built-in adapter path:
...
This keeps `cortex` build-system agnostic while still allowing projects to reuse
...
- Projects can onboard `cortex` around the repository they actually have, not only
  the language set bundled with `cortex`.
...
- Keep `cortex` portable and repo-friendly first.
...
- Add the future `cortex brief` and `external_json` surfaces as first-class
  roadmap items.
...
- project-scw: direction ADR and roadmap for cortex agenting-toolkit work
