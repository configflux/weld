...
Compares the current ``.cortex/graph.json`` against a previous snapshot
...
(``cortex.discover``) just before overwriting the graph.

CLI surface:
    cortex diff                # human-readable summary
    cortex diff --json         # stable JSON contract for agents

MCP surface:
    cortex_diff()              # returns the JSON diff dict
"""
...
def load_and_diff(root: Path) -> dict:
    ...
    from the ``.cortex/`` directory.  If no previous snapshot exists, all
    current nodes are reported as added.  If no current graph exists,
    returns an empty diff.
    """
    cortex_dir = root / ".cortex"
    current_path = cortex_dir / "graph.json"
    previous_path = cortex_dir / "graph-previous.json"
...
def main(argv: list[str] | None = None) -> int:
    """CLI entry point for ``cortex diff``."""
