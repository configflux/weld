#!/usr/bin/env python3
"""Bootstrap a .cortex/discover.yaml by scanning the project."""
...
from cortex._init_ros2 import ros2_source_entries
from cortex.init_detect import (
    detect_ci,
    detect_claude,
    detect_compose,
    detect_dockerfiles,
...
_YAML_HEADER = """\
# .cortex/discover.yaml — Auto-generated by `cortex init`.
#
# Sources are organized by artifact class so the knowledge graph covers the
...
#   tree_sitter       — Tree-sitter AST: exports, types, imports (Go, Rust, TS)
"""
    ...
    for lang, exts in _TREE_SITTER_LANGUAGES.items():
        ...
        for ext in exts:
            buckets["code"].append(_source_entry(
                ...
                comment=f"{lang.capitalize()} sources ({ext})",
                extra=extras,
            ))

    # --- ROS2 (; helpers in cortex/_init_ros2.py) ---
    if ros2_pkg_roots:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="cortex init",
        description="Bootstrap a .cortex/discover.yaml by scanning the project",
    )
    parser.add_argument(
        "root", nargs="?", default=".",
    ...
    parser.add_argument(
        ...
        help="Output path (default: <root>/.cortex/discover.yaml)",
    )
    parser.add_argument(
        "--force", "-f", action="store_true",
        help="Overwrite existing discover.yaml",
    )
    args = parser.parse_args(argv)
    root = Path(args.root).resolve()
    output = Path(args.output) if args.output else root / ".cortex" / "discover.yaml"
