...
- the declaring ``boundary:<path>`` node for the file that sits next to the
  ...
  scheme used by ``cortex/strategies/boundary_entrypoint.py``), so agents can
...
def _owning_service_id(rel_path: str) -> str | None:
    ...
    The topology layer in ``.cortex/discover.yaml`` declares the canonical
...
def _detect_boundary_file(parent_dir: Path, root: Path) -> str | None:
    ...
    check in ``cortex/strategies/boundary_entrypoint.py`` so the two strategies
