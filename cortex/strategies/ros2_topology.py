...
Hard cases — each exercised by ``kg_ros2_topology_cpp_test.py`` and
``kg_ros2_topology_py_test.py`` — include dynamic topic names
...
Low-level helpers live in ``cortex/strategies/_ros2_cpp.py`` (C++ tokens)
and ``cortex/strategies/_ros2_py.py`` (rclpy ``ast`` walker, bd
...
from cortex.strategies import _ros2_cpp as _cpp
...
def extract(root: Path, source: dict, context: dict) -> StrategyResult:
    ...
    from cortex.strategies import _ros2_py as _py  # lazy: avoids import cycle
    nodes: dict[str, dict] = {}
    edges: list[dict] = []
