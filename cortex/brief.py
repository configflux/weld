"""Agent-facing briefing surface for the knowledge graph.
...
def brief(graph: Any, term: str, limit: int = 20) -> dict:
    ...
    Parameters
    ----------
    graph : cortex.graph.Graph
        A loaded Graph instance.
    term : str
        The query term (tokenized search, same as ``cortex query``).
    limit : int
    ...
    # Run the same tokenized query as ``cortex query``.
...
def main(argv: list[str] | None = None) -> None:
    """CLI entry point for ``cortex brief``."""
