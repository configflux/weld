...
Manages ``.cortex/discovery-state.json`` -- a content-hash index that
...
def load_state(root: Path) -> DiscoveryState | None:
    ...
    """
    state_path = root / ".cortex" / STATE_FILENAME
    ...
    try:
        ...
        print(
            f"[cortex] warning: corrupt discovery state file, "
    ...
    if not isinstance(raw, dict):
        print(
            "[cortex] warning: discovery state file is not a JSON object, "
    ...
    if version != STATE_VERSION:
        print(
            f"[cortex] warning: discovery state version mismatch "
    ...
    if not isinstance(files, dict):
        print(
            "[cortex] warning: discovery state 'files' is not a dict, "
            "falling back to full discovery",
            file=sys.stderr,
        )
        return None
