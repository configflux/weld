"""Check Cortex setup status and suggest next steps.

Inspects the .cortex/ directory and prints human-readable guidance so a user
...
def _check_discover_yaml(cortex_dir: Path) -> tuple[list[str], list[str]]:
    lines: list[str] = []
    steps: list[str] = []
    path = cortex_dir / "discover.yaml"
    if not path.is_file():
        line, cmd = _action("discover.yaml not found", "cortex init")
        lines.append(line)
        steps.append(cmd)
    else:
...
def _check_graph_json(cortex_dir: Path, root: Path) -> tuple[list[str], list[str]]:
    lines: list[str] = []
    steps: list[str] = []
    path = cortex_dir / "graph.json"
    if not path.is_file():
        line, cmd = _action("graph.json not found", "cortex discover > .cortex/graph.json")
        lines.append(line)
        steps.append(cmd)
        return lines, steps
    ...
    try:
        ...
        steps.append("cortex discover > .cortex/graph.json")
        return lines, steps
...
def _check_staleness(data: dict, root: Path) -> tuple[list[str], list[str]]:
    ...
    line, cmd = _action(msg, "cortex discover > .cortex/graph.json")
    return [line], [cmd]

def _check_file_index(cortex_dir: Path) -> tuple[list[str], list[str]]:
    lines: list[str] = []
    steps: list[str] = []
    path = cortex_dir / "file-index.json"
    if not path.is_file():
        line, cmd = _action("file-index.json not found", "cortex build-index")
        lines.append(line)
        steps.append(cmd)
    else:
...
def _check_agent_integration(root: Path) -> tuple[list[str], list[str]]:
    claude_cmd = root / ".claude" / "commands" / "cortex.md"
    codex_skill = root / ".codex" / "skills" / "cortex" / "SKILL.md"
    if claude_cmd.is_file() or codex_skill.is_file():
        return [], []
    lines = [_status("INFO", "No agent integration found — run: cortex bootstrap claude  (or: codex)")]
    return lines, []
...
def prime(root: Path) -> str:
    ...
    if not cortex_dir.is_dir():
        return (
            "Cortex is not set up yet.\n"
            "\n"
            "Get started:\n"
            "  1. cortex init                       # bootstrap .cortex/discover.yaml\n"
            "  2. cortex discover > .cortex/graph.json   # build the graph\n"
            "  3. cortex build-index                 # build the keyword index\n"
            "  4. cortex bootstrap claude             # or: cortex bootstrap codex\n"
            "  5. cortex prime                        # confirm everything is set up\n"
        )
    ...
    for check in (
        lambda: _check_discover_yaml(cortex_dir),
        lambda: _check_graph_json(cortex_dir, root),
        lambda: _check_file_index(cortex_dir),
    ...
    header = f"Cortex Status (.cortex/ in {root})\n"
    body = "\n".join(all_lines)
    ...
    return f"{header}\n{body}\n\nKG is up to date. No actions needed.\n"

def main(argv: list[str] | None = None) -> None:
    """CLI entry point for ``cortex prime``."""
    parser = argparse.ArgumentParser(
        prog="cortex prime",
        description="Check Cortex setup status and suggest next steps",
    )
    parser.add_argument(
