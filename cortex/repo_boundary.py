"""Shared repo-boundary helpers for Cortex discovery, init, and indexing.
...
# Directory names that are always excluded from Cortex discovery results or the
...
def is_excluded_dir_name(name: str) -> bool:
    """Return True when *name* is always outside the cortex repo boundary."""
...
def path_within_repo_boundary(
    ...
    *,
    ...
    """Return True when *path* is inside cortex's repo-visible boundary."""
    root = root.resolve()
    try:
...
def filter_repo_paths(
    ...
    *,
    ...
    """Filter *paths* down to cortex-visible repo paths."""
    return [
        path
        for path in paths
