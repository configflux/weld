...
wd bootstrap codex --cli-only   # set up Codex integration (CLI only)
```

This bootstrap was produced with `--cli-only`, so Codex invokes `wd` as a
shell command and no server config file is written.
