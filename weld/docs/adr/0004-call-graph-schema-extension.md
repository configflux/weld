...
on?". The current `weld` graph cannot answer those questions at function
...
[weld/contract.py](../../contract.py) is used only by the Bazel and
...
structured query that the weld MCP server
...
- `file` is a coarse artifact node. Attaching per-function call lists as
  props breaks the "one concept per node" model the rest of weld relies
...
- `entity` is already used for ORM entities and top-level Python classes
  via [weld/strategies/python_module.py](../../strategies/python_module.py);
  widening its meaning would muddy existing queries.
- A new node type keeps symbols cleanly queryable (`wd query symbol:foo`),
...
(for example `symbol:py:weld.strategies.tree_sitter:_load_language`).
...
Add `calls` to `VALID_EDGE_TYPES` in [weld/contract.py](../../contract.py).
...
- **Python**: new `weld/strategies/python_callgraph.py` using stdlib `ast`.
...
- **TypeScript / Go / Rust**: extend
  [weld/strategies/tree_sitter.py](../../strategies/tree_sitter.py) and the
  per-language query files in [weld/languages/](../../languages/) with a new
...
Bump `SCHEMA_VERSION` in [weld/contract.py](../../contract.py) from `1` to
`2`. Existing `graph.json` files are not migrated in place; the migration
story is "re-run `wd discover`". The orchestrator rejects graphs with an
...
- No new CLI surface in this ADR. `wd callers` / `wd references` and the
  matching MCP tools `weld_callers` / `weld_references` are an implementation
...
- The graph now contains best-effort data. Consumers must distinguish
  ...
  truth.
- Schema version bump forces a one-time `wd discover` for every consumer
  on upgrade.
