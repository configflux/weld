...
in a single-file discovery entrypoint that was 1154 lines and contained 13
strategy functions, a YAML parser, shared AST helpers, and the discovery
orchestrator -- all in one file.
...
module. A user adopting weld for a new project cannot add extraction logic
without forking the tool. The plan to make weld portable and shippable to
...
1. **Portability** -- strategies must be loadable without editing weld source.
2. **Zero mandatory dependencies** -- bundled strategies must work with
   stdlib only (no pip install required for core operation).
3. **Build-system agnostic** -- weld must not assume Bazel, Make, or any
...
5. **Security** -- loading arbitrary Python from `.weld/strategies/` is
...
`weld/strategies/*.py` but hardcode imports in the discovery runtime. This solves
...
plugin mechanism. Requires packaging and installation. Rejected because weld
...
`weld/strategies/` and `.weld/strategies/` to `sys.path`, then
...
1. `.weld/strategies/foo.py` (project-local override -- checked first so
   projects can shadow bundled strategies)
2. `weld/strategies/foo.py` (bundled with tool)
...
```python
spec = importlib.util.spec_from_file_location(f"weld_strategy_{name}", path)
...
that are used by multiple strategies move to `weld/strategies/_helpers.py`.
...
- Adding a new extraction strategy requires only dropping a `.py` file in
  the right directory -- no changes to weld source.
