...
The weld tool's portability goal requires extracting symbols (functions,
...
1. **Build-system agnostic** -- weld must work as a plain directory copy or
   `pip install`. It cannot require Bazel, Make, or native compilation at
   the user's site.
2. **Zero mandatory dependencies** -- the core weld tool (graph engine,
...
4. **Install friction** -- `pip install tree-sitter tree-sitter-python
   tree-sitter-typescript tree-sitter-go tree-sitter-c-sharp` is
   straightforward on any platform
...
extraction, but still discoverable via `wd find`.
...
```
Historical runtime layout (stdlib only): single-file graph/discovery entrypoints plus bundled strategies
...
Tree-sitter queries are bundled as YAML files in `weld/languages/`:
...
types, and imports from that language. Adding a new language usually requires
only a new YAML file; language-specific dependency edges or metadata can use a
small helper beside the shared strategy.
...
`wd init` will detect whether tree-sitter is installed and generate the
...
- Adding language support requires only a YAML query file in
  `weld/languages/`, not a new Python strategy.
...
- Users on constrained environments (no C compiler, corporate lockdown) can
  still use weld with reduced extraction quality.
...
- Users must understand that `pip install tree-sitter` unlocks better
  extraction. Mitigation: `wd discover` prints a clear message when
...
- Tree-sitter grammar package versions must track upstream. Mitigation:
  pin versions in a `requirements-weld.txt` or document compatible ranges.
...
- Core weld functionality (graph, find, stale, init) works without tree-sitter.
- The `python_module` strategy (stdlib `ast`) remains the Python fallback
  indefinitely -- it is not deprecated by tree-sitter.
- Language query files in `weld/languages/` are maintained alongside the
