"""C++ cross-file include resolver (layer 2 of weld-cpp-ros2).

Layer 1 (``weld/strategies/tree_sitter.py``) emits
...
def _ts_module_from_path(rel_path: str) -> str:
    ...
    Duplicated from ``weld.strategies.tree_sitter._ts_module_from_path``
...
def augment_state_with_headers(
    ...
    root_resolved = root.resolve()
    # Iterate in sorted order: CPP_HEADER_EXTS is a frozenset of strings
    # whose native iteration order depends on the per-process hash seed
    # (ADR 0012 §2 row 1). A deterministic order here matters because
    # header parses are appended to per_file in this loop's order and
    # that list drives downstream edge rewrites.
