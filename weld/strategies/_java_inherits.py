...
def emit_inheritance_edges(
    ...
    * Otherwise -- ``symbol:unresolved:<base_short>`` sentinel, minted
    ...
    for record in inheritance_records:
        ...
        if rel_path:
            # ADR 0074: attribute the edge to the file whose extends/
            # implements clause produced it -- record_inheritance already
            # captures rel_path per record, one file per declaration
            # (bd rifzk).
