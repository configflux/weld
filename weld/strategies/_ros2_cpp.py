...
used by ``weld/strategies/ros2_topology.py``.  It exists so the strategy
...
- config loading for ``weld/languages/cpp_ros2.yaml``
...
def base_is_ros_node(
    ...
    *node_bases* is the ``node_bases`` list loaded from
    ``weld/languages/cpp_ros2.yaml``; each entry is a ``{match, lifecycle}``
...
def load_cpp_ros2_config() -> dict:
    """Load ``weld/languages/cpp_ros2.yaml``; empty dict on any failure."""
