...
# weld CLI test: exercises graph engine commands against a temporary graph.
...
# --- add-node ---
out="$(wd_cmd add-node entity:Store --type entity --label Store --props '{"table":"store"}')"
echo "${out}" | python3 -c "import json,sys; d=json.load(sys.stdin); assert d['id']=='entity:Store', f'add-node id: {d}'"

out="$(wd_cmd add-node entity:Source --type entity --label Source --props '{}')"
echo "${out}" | python3 -c "import json,sys; d=json.load(sys.stdin); assert d['id']=='entity:Source'"

out="$(wd_cmd add-node entity:Flyer --type entity --label Flyer --props '{}')"

# --- add-edge ---
out="$(wd_cmd add-edge entity:Source entity:Store --type depends_on --props '{"fk":"store.id"}')"
...
# --- rm-edge ---
out="$(wd_cmd rm-edge entity:Source entity:Store --type depends_on)"
echo "${out}" | python3 -c "import json,sys; d=json.load(sys.stdin); assert d['removed_count']==1"

# --- rm-node ---
out="$(wd_cmd rm-node entity:Flyer)"
...
SEED
out="$(wd_cmd import "${TMPDIR}/import.json")"
...
echo "PASS: all weld CLI tests passed"
