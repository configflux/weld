...
# Unit tests for the weld plugin strategy architecture.
...
python3 -c "
import sys; sys.path.insert(0, '${ROOT}')
from weld.strategies._helpers import base_names, inherits, tablename, extract_fks
...
import sys; sys.path.insert(0, '${ROOT}')
import importlib
mod = importlib.import_module('weld.strategies.${strat}')
...
from weld.discover import _load_strategy
...
mkdir -p "${TMPDIR}/shadow_test/.weld/strategies"
cat > "${TMPDIR}/shadow_test/.weld/strategies/markdown.py" << 'PYEOF'
from weld.strategies._helpers import StrategyResult
...
# Context keys are owned by the inner helper _discover_single_repo; the
# public discover() is a thin dispatcher per ADR 0011.
python3 -c "
import sys; sys.path.insert(0, '${ROOT}')
from weld.discover import _discover_single_repo
doc = _discover_single_repo.__doc__ or ''
...
# typescript_exports entries. In standalone weld repos,
# there is no apps/web directory and thus no typescript_exports config,
# so this test is skipped.
if [[ -f "${ROOT}/.weld/discover.yaml" && -d "${ROOT}/apps/web" ]]; then
  python3 -c "
import sys; sys.path.insert(0, '${ROOT}')
from weld._yaml import parse_yaml
from pathlib import Path
config = parse_yaml(Path('${ROOT}/.weld/discover.yaml').read_text(encoding='utf-8'))
...
for name in ['.git', 'node_modules', '__pycache__', '.weld', 'bazel-bin',
...
# Need to initialize weld importability
mkdir -p "${TMPDIR}/strat_glob_test/.weld"
...
echo "PASS: all weld strategy unit tests passed"
