...
class ImpactMcpTest(unittest.TestCase):
    ...
    def test_tool_registered_and_dispatch_matches_helper(self) -> None:
        ...
        # Dispatch stamps the additive freshness object (bd 85tb.3); the impact
        # envelope is otherwise identical to the bare handler result.
        self.assertIn("freshness", dispatch_result)
        self.assertEqual(
            {k: v for k, v in dispatch_result.items() if k != "freshness"},
            helper_result,
        )
