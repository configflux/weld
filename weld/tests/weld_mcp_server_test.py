"""Tests for ``weld.mcp_server`` -- the stdio MCP adapter over wd query helpers.
...
_FIXTURE_NODES: dict[str, dict] = {
    ...
    "doc:adr/0015-weld-mcp": {
        "type": "doc",
        "label": "ADR 0015 Weld MCP",
        "props": {
            "file": "docs/adrs/0015-weld-mcp-server-exposure.md",
            "doc_kind": "adr",
            "description": "Architecture decision for the Weld MCP server.",
        },
    },
}
...
class WeldMcpServerToolsTest(unittest.TestCase):
    ...
    def test_weld_query_returns_query_envelope(self) -> None:
    ...
    def test_weld_find_returns_file_envelope(self) -> None:
        result = mcp_server.weld_find("store", root=self.root)
        self.assertEqual(result["query"], "store")
    ...
    def test_weld_find_matches_cli_helper(self) -> None:
    ...
    def test_weld_context_returns_node_and_neighborhood(self) -> None:
        result = mcp_server.weld_context("entity:Store", root=self.root)
    ...
    def test_weld_context_missing_node_returns_error(self) -> None:
        result = mcp_server.weld_context("entity:DoesNotExist", root=self.root)
        self.assertIn("error", result)

    # ---- weld_path ------------------------------------------------------

    def test_weld_path_finds_shortest_path(self) -> None:
        result = mcp_server.weld_path(
            "route:GET:/api/v1/stores",
            "entity:Offer",
            root=self.root,
        )
    ...
    def test_weld_path_unknown_node_returns_none(self) -> None:
        result = mcp_server.weld_path(
            "entity:Store",
            "entity:Nope",
            root=self.root,
    ...
    def test_weld_brief_returns_versioned_envelope(self) -> None:
    ...
    def test_weld_callers_returns_direct_callers(self) -> None:
    ...
    def test_weld_references_combines_callers_and_files(self) -> None:
        result = mcp_server.weld_references("helper", root=self.root)
        match_ids = {m["id"] for m in result["matches"]}
    ...
    def test_weld_stale_returns_freshness_shape(self) -> None:
        result = mcp_server.weld_stale(root=self.root)
...
class WeldMcpServerRegistryTest(unittest.TestCase):
    ...
    def test_tool_input_schemas_declare_required_args(self) -> None:
        ...
        self.assertEqual(by_name["weld_query"].input_schema["required"], ["term"])
        self.assertEqual(by_name["weld_find"].input_schema["required"], ["term"])
        self.assertEqual(by_name["weld_context"].input_schema["required"], ["node_id"])
        self.assertEqual(
            by_name["weld_path"].input_schema["required"],
            ["from_id", "to_id"],
        )
        self.assertEqual(by_name["weld_brief"].input_schema["required"], ["area"])
        self.assertEqual(by_name["weld_stale"].input_schema.get("required", []), [])
        self.assertEqual(
            by_name["weld_callers"].input_schema["required"], ["symbol_id"]
        )
        self.assertEqual(
            by_name["weld_references"].input_schema["required"], ["symbol_name"]
        )
        self.assertEqual(by_name["weld_enrich"].input_schema.get("required", []), [])

    def test_dispatch_weld_query_happy_path(self) -> None:
        result = mcp_server.dispatch("weld_query", {"term": "Store"}, root=self.root)
        self.assertIn("matches", result)
        match_ids = {m["id"] for m in result["matches"]}
        self.assertIn("entity:Store", match_ids)

    def test_dispatch_weld_brief_happy_path(self) -> None:
        result = mcp_server.dispatch("weld_brief", {"area": "Store"}, root=self.root)
        self.assertEqual(result["brief_version"], BRIEF_VERSION)

    def test_dispatch_unknown_tool_raises(self) -> None:
        with self.assertRaises(KeyError):
            mcp_server.dispatch("weld_nope", {}, root=self.root)

    def test_import_does_not_require_mcp_sdk(self) -> None:
        # Importing weld.mcp_server must succeed even when the optional ``mcp``
