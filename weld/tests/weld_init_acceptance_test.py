"""Acceptance tests for wd init — whole-repo onboarding via fixture repos.

Validates that wd init produces a discover.yaml that:
...
Uses the polyglot fixture repos under weld/tests/fixtures/.
...
class InitArtifactClassCoverageTest(unittest.TestCase):
    ...
    def test_csharp_project_has_all_artifact_classes(self) -> None:
        text, _ = _init_fixture("csharp_project")
        self._assert_all_sections(text, "csharp_project")
...
class InitStrategyDetectionTest(unittest.TestCase):
    ...
    def test_csharp_project_emits_tree_sitter_csharp(self) -> None:
        _, data = _init_fixture("csharp_project")
        csharp_sources = [
            s for s in data.get("sources", [])
            if s.get("strategy") == "tree_sitter"
            and s.get("language") == "csharp"
        ]
        self.assertTrue(
            csharp_sources,
