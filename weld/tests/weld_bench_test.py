...
text files containing the search term) and assert that:
  ...
  - Both ``weld_cli_baseline`` and ``weld_mcp_baseline`` produce non-empty
  ...
  - ``render_report`` produces the documented markdown shape with a
    summary section.
  - The ``wd bench`` CLI subcommand writes a report file at the requested
    path.
...
from weld.bench.runner import (  # noqa: E402
    BenchResult,
    Prompt,
    count_tokens,
    grep_baseline,
...
def _setup_repo(term: str = "Store") -> str:
    """Create a temp repo dir with .weld/graph.json + a few text files."""
    ...
    with open(os.path.join(weld_dir, "graph.json"), "w") as f:
        json.dump(graph, f)
    # Minimal file index so weld_find/references calls don't crash.
...
class PromptLoaderTest(unittest.TestCase):
    ...
    def test_real_fixture_loads(self) -> None:
        ...
        path = Path(_repo_root) / "weld" / "tests" / "bench" / "prompts.yaml"
        prompts = load_prompts(path)
...
class WeldBaselinesTest(unittest.TestCase):
    def test_weld_cli_baseline_returns_json_for_each_category(self) -> None:
        root = Path(_setup_repo("Store"))
        for cat in ("navigation", "dependency", "callgraph"):
            ...
            out = weld_cli_baseline(prompt, root)
            # Must be valid JSON and non-empty.
            parsed = json.loads(out)
            self.assertIsInstance(parsed, dict)
            self.assertGreater(count_tokens(out), 0)

    def test_weld_mcp_baseline_returns_json_for_each_category(self) -> None:
        root = Path(_setup_repo("Store"))
        for cat in ("navigation", "dependency", "callgraph"):
            prompt = Prompt(
                id="q",
            ...
            out = weld_mcp_baseline(prompt, root)
            parsed = json.loads(out)
            self.assertIsInstance(parsed, dict)
            self.assertGreater(count_tokens(out), 0)
...
class RunBenchTest(unittest.TestCase):
    ...
    def test_render_report_includes_summary(self) -> None:
        ...
        self.assertIn("# Weld retrieval benchmark", report)
        self.assertIn("| id ", report)
        self.assertIn("## Summary", report)
        self.assertIn("weld CLI vs grep", report)
        self.assertIn("weld MCP vs grep", report)
...
class CliEntrypointTest(unittest.TestCase):
    def test_weld_bench_writes_report_file(self) -> None:
        root = Path(_setup_repo("Store"))
        ...
        self.assertIn("# Weld retrieval benchmark", text)
        self.assertIn("| q01 | navigation", text)

    def test_weld_bench_print_mode(self) -> None:
        root = Path(_setup_repo("Store"))
        prompts_path = root / "prompts.yaml"
        prompts_path.write_text(_FIXTURE_PROMPTS, encoding="utf-8")
        buf = io.StringIO()
        ...
        self.assertEqual(rc, 0)
        self.assertIn("Weld retrieval benchmark", buf.getvalue())
