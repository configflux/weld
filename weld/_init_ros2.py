...
wiring into ``wd init`` is this task.
"""
...
def _entry(
    ...
    *, comment: str, extra: dict[str, str] | None = None,
    ...
    """Return a YAML source-entry block matching ``weld/init._source_entry``."""
    lines: list[str] = [f"\n  # --- {comment} ---"]
...
def ros2_source_entries(pkg_roots: list[str]) -> list[str]:
    ...
    modules in ``weld/strategies/ros2_*.py``).
    """
    entries: list[str] = []
    ...
    # (see ``weld/strategies/ros2_topology.py``), so emit one glob per
