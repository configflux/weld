...
``.weld/graph.json`` is content-addressable except for a few volatile meta
...
* ``meta.git_branch`` -- the branch discovery ran on (ADR 0096 §3;
  absent outside a git checkout and on a detached ``HEAD``). Branch is
  *checkout* identity, not content: two worktrees on different branches
  at the same commit have identical sources, so keeping it volatile is
  what preserves byte-identical ``graph.json`` across them.

ADR 0065 relocates those fields into a sibling, **gitignored** sidecar
``.weld/graph-meta.json`` so two ``wd discover`` runs at a fixed commit --
including runs from two different worktrees on two different branches --
...
# ADR 0096 §3). Order is irrelevant for correctness; kept stable for
# readability. Adding a key here is the *only* supported way to keep a
# meta field out of the content-addressable graph body -- every writer
# funnels through ``write_graph_with_meta`` and every reader through
# ``merge_sidecar_meta``, so no call site needs to learn the new key.
VOLATILE_META_KEYS: tuple[str, ...] = ("updated_at", "git_sha", "git_branch")
...
def _on_disk_matches(path: Path, candidate: bytes) -> bool:
    ...
    The size check first: a differing length settles it without reading the
    file at all, which is the common case for the callers that *changed* the
    graph (``wd add-node``, an enrichment write). They would otherwise pay a
    full re-read to learn what one ``stat`` already knows (bd emyk).

    Newline translation, if a platform ever applies any, can only make this
    answer ``False`` for bytes that are on disk in a translated form -- the
    write below goes through ``atomic_write_text``. That direction is safe:
    it costs a rewrite that would have happened anyway, and never skips one
    that was needed.
    """
    try:
        if not path.is_file():
            return False
        if path.stat().st_size != len(candidate):
            return False
        import hashlib
...
def write_graph_with_meta(
    ...
    **Identical bytes are never rewritten**, whoever the caller is (bd emyk).
    The check was previously reachable only through the one caller that
    passed *on_disk_bytes*, which left ``Graph.save``, the ``wd discover``
    CLI tail, ``wd warm`` and the federated writers rewriting a
    byte-identical graph on every run. Skipping is what holds the *inode*
    still, and that is the point: weld writes by atomic rename, so an
    identical-bytes rewrite lands a fresh ``st_mtime_ns`` and invalidates
    every cheap freshness check keyed on it -- the ``published_graph`` token
    (forcing ``discovery-state.json`` to re-stamp its ``mtime_ns``, one line
    of git diff per no-change discover in Mode B), the sidecar's
    ``graph_mtime_ns`` mirror, and the digest memos. The sidecar below
    re-stats this same untouched file, so its mirror still pins it.
    ...
    if on_disk_bytes is None:
        # No caller-supplied bytes: serialize here, then take the *same*
        # content check. One funnel, one decision (bd emyk).
        on_disk_bytes = _dumps_graph(on_disk).encode("utf-8")
        body_matches_disk = False
