"""First-context quality scoring for weld retrieval surfaces.

Measures whether ``wd brief`` and ``wd trace`` return relevant, well-bucketed
results for representative agent queries. Complements the token-cost harness
in :mod:`weld.bench.runner` by adding **relevance** and **bucket coverage**
dimensions.
...
The harness is on-demand, not a CI gate. It runs via ``wd bench --quality``
...
from weld.bench.runner import count_tokens
...
def render_quality_report(results: list[QualityResult]) -> str:
    ...
    lines: list[str] = [
        "# Weld first-context quality benchmark",
        "",
        "Measures whether `wd brief` and `wd trace` return relevant, "
        "well-bucketed results for representative agent queries.",
        "",
        "See `weld/bench/quality.py` for the scoring engine and "
        "`weld/tests/bench/quality_cases.yaml` for the fixture.",
        "",
