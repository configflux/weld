"""Write bundled Weld starter templates into the current repository."""
...
_DEFAULT_OUTPUT_DIRS = {
    "external-adapter": Path(".weld") / "adapters",
    "local-strategy": Path(".weld") / "strategies",
}
...
def main(argv: list[str] | None = None) -> None:
    """CLI entry point for ``wd scaffold``."""
    parser = argparse.ArgumentParser(
        prog="wd scaffold",
        description="Write bundled Weld templates into the current repository",
    )
    ...
    for kind, help_text in kinds:
        ...
        kind_parser.add_argument(
            ...
            type=Path,
            default=None,
            help="Custom output path (defaults under .weld/)",
        )
        kind_parser.add_argument(
            "--force",
            action="store_true",
